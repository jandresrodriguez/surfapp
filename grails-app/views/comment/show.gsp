	...
	<body>
		...
		<div id="show-comment" class="content scaffold-show" role="main">
			...
			<ol class="property-list comment">
				...
				<g:if test="${commentInstance?.parent}">
				<li class="fieldcontain">
					<span id="parent-label" class="property-label"><g:message code="comment.parent.label" default="Parent" /></span>
					
						<span class="property-value" aria-labelledby="parent-label"><g:link controller="post" action="show" id="${commentInstance?.parent?.id}">${commentInstance?.parent?.encodeAsHTML()}</g:link></span>
